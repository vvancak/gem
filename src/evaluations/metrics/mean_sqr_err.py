...
def mse_new_obs_at_k(graph: nx.Graph, learning_graph: nx.Graph, embedding: eb.EmbeddingBase, max_k: int, ns_size: int = 10) -> t.Dict:
    num_correct = 0
    err = 0.0

    errors = []
    emb_edges, graph_edges = ens.get_top_new_edges_ns(graph, learning_graph, embedding, max_k, ns_size)
    for (ew, (u, v)), (gw, (_, _)) in zip(emb_edges, graph_edges):
        if graph.has_edge(u, v):
            err += (ew - gw) ** 2
            num_correct += 1
